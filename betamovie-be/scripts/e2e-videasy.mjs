...
/**
 * End-to-end check for Videasy provider:
 * 1) backend stream endpoint returns one or more streams
 * 2) returned HLS/file stream survives proxy fetch
 * 3) at least one segment/media response contains playable binary bytes
 *
 * Usage:
 *   node scripts/e2e-videasy.mjs --type movie --id 550
 *   node scripts/e2e-videasy.mjs --type tv --id 82452 --season 2 --episode 1
 */
...
const streamEndpoint =
  type === 'movie'
    ? `${backendBase}/api/embed/api/streams/videasy/movie/${encodeURIComponent(id)}`
    : `${backendBase}/api/embed/api/streams/videasy/tv/${encodeURIComponent(id)}/${season}/${episode}`;
...
const normalizedLines = manifest =>
  manifest
    .split(/\r?\n/)
    .map(line => line.trim())
    .filter(Boolean);
...
const firstUri = manifest =>
  normalizedLines(manifest).find(line => !line.startsWith('#') && !line.startsWith('<')) || '';

const nextUriAfterTag = (manifest, tagPrefix) => {
  const lines = normalizedLines(manifest);
...
const pickChildManifestLine = manifest => nextUriAfterTag(manifest, '#EXT-X-STREAM-INF') || '';

const pickSegmentLine = manifest => nextUriAfterTag(manifest, '#EXTINF') || firstUri(manifest);
...
if (!streamPayload?.success || !streamPayload?.count || !returnedStreams.length) {
  console.error('No playable Videasy stream returned from backend provider');
...
for (const stream of returnedStreams) {
  ...
  let mediaPlaylistUrl = candidateUrl;
  let mediaPlaylistText = playlistText;

  const childManifestLine = pickChildManifestLine(playlistText);
  if (childManifestLine) {
    mediaPlaylistUrl = withToken(new URL(childManifestLine, candidateUrl).toString());
    const childResponse = await fetch(mediaPlaylistUrl, {
      headers: { Accept: '*/*' },
    }).catch(() => null);
    if (!childResponse?.ok) {
      continue;
    ...
    mediaPlaylistText = await childResponse.text();
    if (!mediaPlaylistText.trimStart().startsWith('#EXTM3U')) {
      continue;
    }
  }

  const segmentLine = pickSegmentLine(mediaPlaylistText);
  if (!segmentLine) {
    continue;
  }

  const segmentUrl = withToken(new URL(segmentLine, mediaPlaylistUrl).toString());
...
if (!verified) {
  console.error('No playable Videasy stream survived proxy validation');
  console.error(JSON.stringify(streamPayload));
  process.exit(1);
}
...
if (verified.mediaPlaylistUrl) {
  console.log(`media playlist url: ${verified.mediaPlaylistUrl}`);
}
