...
const VIDSRC_WTF_API_BASE = `${getBaseUrl()}/api/streams/vidsrcwtf`;

interface VidSrcWtfStream {
  name: string;
  title: string;
  url: string;
  subtitle?: string;
...
interface VidSrcWtfApiResponse {
  success: boolean;
  tmdbId: string;
  imdbId: string | null;
  count: number;
  providerTimings: Record<string, number>;
  streams: VidSrcWtfStream[];
}
...
function mapStreamsToEmbeds(streams: VidSrcWtfStream[]) {
...
function encodeStreamInfo(stream: VidSrcWtfStream): string {
  const info = {
    name: stream.name,
    title: stream.title,
    url: stream.url,
...
export async function scrapeVidSrcWtfMovie(
  ...
  try {
    for (const mediaId of candidateIds) {
      const apiUrl = `${VIDSRC_WTF_API_BASE}/movie/${mediaId}`;
      const data = await ctx.fetcher<VidSrcWtfApiResponse>(apiUrl, {
        credentials: "include",
      });
    ...
    throw new NotFoundError("No streams found on VidSrc.wtf");
  } catch (err) {
    console.error("[VidSrc.wtf] Scrape failed:", err);
    if (err instanceof NotFoundError) throw err;
    throw new NotFoundError("Failed to fetch streams from VidSrc.wtf");
  }
}

export async function scrapeVidSrcWtfShow(
  ...
  try {
    for (const mediaId of candidateIds) {
      const apiUrl = `${VIDSRC_WTF_API_BASE}/tv/${mediaId}/${ctx.media.season.number}/${ctx.media.episode.number}`;
      const data = await ctx.fetcher<VidSrcWtfApiResponse>(apiUrl, {
        credentials: "include",
      });
    ...
    console.error("[VidSrc.wtf] Show scrape failed:", err);
    if (err instanceof NotFoundError) throw err;
    throw new NotFoundError("Failed to fetch streams from VidSrc.wtf");
  }
}
