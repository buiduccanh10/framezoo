...
const MOVIES111_API_BASE = `${getBaseUrl()}/api/streams/111movies`;

interface Movies111Stream {
  name: string;
  title: string;
  url: string;
  subtitle?: string;
...
interface Movies111ApiResponse {
  success: boolean;
  tmdbId: string;
  imdbId: string | null;
  count: number;
  providerTimings: Record<string, number>;
  streams: Movies111Stream[];
}
...
function encodeStreamInfo(stream: Movies111Stream): string {
  const info = {
    name: stream.name,
    title: stream.title,
    url: stream.url,
...
export async function scrape111MoviesMovie(
  ctx: MovieScrapeContext,
): Promise<SourcererOutput> {
  ctx.progress(10);

  const apiUrl = `${MOVIES111_API_BASE}/movie/${ctx.media.tmdbId}${buildContextQuery(ctx)}`;

  try {
    const data = await ctx.fetcher<Movies111ApiResponse>(apiUrl, {
      credentials: "include",
    });

    if (!data.success || !data.streams?.length) {
      throw new NotFoundError("No streams found on 111Movies");
    }
    ...
    console.error("[111Movies] Scrape failed:", err);
    if (err instanceof NotFoundError) throw err;
    throw new NotFoundError("Failed to fetch streams from 111Movies");
  }
}

export async function scrape111MoviesShow(
  ctx: ShowScrapeContext,
): Promise<SourcererOutput> {
  ctx.progress(10);

  const apiUrl = `${MOVIES111_API_BASE}/tv/${ctx.media.tmdbId}/${ctx.media.season.number}/${ctx.media.episode.number}${buildContextQuery(ctx)}`;
  ...
  try {
    ...
    console.error("[111Movies] Show scrape failed:", err);
    if (err instanceof NotFoundError) throw err;
    throw new NotFoundError("Failed to fetch streams from 111Movies");
  }
}
