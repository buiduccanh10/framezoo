...
const SKIP_LOCALES = new Set(["en", "futhark", "kitty", "minion", "nv", "pirate", "tok", "umb", "uwu"]);
...
function buildSchemaOnlyFromEn(enObj) {
  ...
  if (isObject(enObj)) {
    const out = {};
    for (const [k, v] of Object.entries(enObj)) out[k] = buildSchemaOnlyFromEn(v);
    return out;
  }
  return enObj;
}
...
async function main() {
  ...
  for (const file of pickedFiles) {
    ...
    if (SKIP_LOCALES.has(locale)) {
      console.log(`[${localeIndex}/${pickedFiles.length}] ${locale}: skipped (custom locale)`);
      continue;
    }
