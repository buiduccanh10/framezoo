...
const PipPlaybackClockContext = createContext<number | null>(null);

function isPipClockActive(state: DesktopPipState): boolean {
  return (
    state.isPlaying &&
    !state.paused &&
    !state.isSeeking &&
    !state.isLoading &&
    state.hasRenderedFrame &&
...
function PipPlaybackClockProvider(props: {
  ...
  const time = useSmoothPlaybackClock({
    ...
    isActive: isPipClockActive(props.state),
    isSeeking: props.state.isSeeking,
    resetKey: [
      props.state.episode?.season ?? "",
      props.state.episode?.episode ?? "",
      props.state.source?.type ?? "",
      props.state.source?.url ?? "",
    ].join("|"),
  });
...
function usePipSharedPlaybackClock(): number {
  const time = useContext(PipPlaybackClockContext);
  if (time === null) {
    throw new Error(
      "usePipSharedPlaybackClock must be used inside PipPlaybackClockProvider",
    );
  }
  return time;
}
