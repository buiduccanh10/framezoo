...
export function DocumentPipOverlay() {
  ...
  const title = meta?.episode?.title || meta?.title || "AlphaFlix";
  ...
  useEffect(() => {
    ...
    root.style.setProperty(
      "--betamovie-document-pip-subtitle-bottom",
      controlsVisible ? "3.55rem" : "1.15rem",
    );
    root.style.setProperty(
      "--betamovie-pip-subtitle-background",
      "rgba(0,0,0,0.68)",
    );
    root.style.setProperty("--betamovie-pip-subtitle-font-weight", "600");
    root.style.setProperty(
      "--betamovie-pip-subtitle-text-shadow",
      "0 2px 5px rgba(0,0,0,0.96)",
    );

    return () => {
      root.style.removeProperty("--betamovie-document-pip-subtitle-bottom");
      root.style.removeProperty("--betamovie-pip-subtitle-background");
      root.style.removeProperty("--betamovie-pip-subtitle-font-weight");
      root.style.removeProperty("--betamovie-pip-subtitle-text-shadow");
    };
